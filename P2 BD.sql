...
FROM [software] s
WHERE s.idSoftware IN
(
	SELECT sm.idSoftware
	FROM [usuario] u, [depto] d, [maquina] m, [usuarioMaquina] um, [softwareMaquina] sm
	WHERE d.depto = 'Desenvolvimento'
	AND m.idMaquina = um.idMaquina
	AND sm.idMaquina = sm.idSoftware
	AND u.idUsuario = um.idUsuario
)
